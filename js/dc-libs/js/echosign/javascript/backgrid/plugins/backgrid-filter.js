...
*/
/*
  INCLUDES ADOBE changes. Please migrate the changes when the file is updated with newer version.
 */
...
}(window, function (_, Backbone, Backgrid, lunr) {
  ...
  */
  var ClientSideFilter = Backgrid.Extension.ClientSideFilter = ServerSideFilter.extend({
    ...
    events: _.extend({}, ServerSideFilter.prototype.events, {
      ...
      "submit": function (e) {
        e.preventDefault();
        // start: AODBE change (passing the event in the search function for accessibility check)
        this.search(e);
        // end: ADOBE change
      }
    }),
